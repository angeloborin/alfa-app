...
import { 
  ...
  CheckSquare, CalendarDays, Receipt, Eye, EyeOff
} from 'lucide-react';
...
// --- COMPONENTES DE GRÁFICO (SIMPLES & LEVES) ---
...
export default function App() {
  ...
  const [showValues, setShowValues] = useState(true); // ESTADO PARA CONTROLAR VISIBILIDADE DE VALORES
  ...
  const globalFinancials = useMemo(() => {
    ...
    const topClients = Object.entries(revenueByClient)
      .map(([label, value]) => ({ 
        label, 
        value, 
        // Correção aqui: min e max ajustados para 2 para evitar RangeError
  ...
  useEffect(() => {
    if (!user) return;
    const unsubOrders = onSnapshot(collection(db, 'artifacts', finalAppId, 'public', 'data', 'serviceOrders'), (snap) => {
      const list = snap.docs.map(d => ({ firestoreId: d.id, ...d.data() }));
      setOrders(list.sort((a, b) => b.osNumber?.localeCompare(a.osNumber)));
      setIsLoading(false);
    });
    const unsubContracts = onSnapshot(collection(db, 'artifacts', finalAppId, 'public', 'data', 'contracts'), (snap) => {
        setContracts(snap.docs.map(d => d.data()));
    });
    return () => { unsubOrders(); unsubContracts(); };
  }, [user]);
  ...
  const openModal = (order = null) => {
    setTempSolution(''); setTempCost(''); setIsFinancialOpen(false);
    if (order) {
  ...
  const handleSaveContract = async (e) => {
      e.preventDefault();
      if(!contractForm.clientName.trim()) { alert("Nome do cliente é obrigatório"); return; }
      setIsSaving(true);
      try {
  ...
  const confirmDelete = (order) => { setOrderToDelete(order); setIsDeleteModalOpen(true); };
  const handleDelete = async () => { if (!orderToDelete) return; try { await deleteDoc(doc(db, 'artifacts', finalAppId, 'public', 'data', 'serviceOrders', orderToDelete.firestoreId)); setIsDeleteModalOpen(false); setOrderToDelete(null); } catch (err) { alert("Erro ao excluir."); } };
  const addSolutionItem = () => { if (!tempSolution.trim()) return; setFormData(prev => ({ ...prev, solutionsList: [...prev.solutionsList, { id: Date.now(), text: tempSolution, cost: tempCost || "0,00" }] })); setTempSolution(''); setTempCost(''); };
  const removeSolutionItem = (id) => { setFormData(prev => ({ ...prev, solutionsList: prev.solutionsList.filter(item => item.id !== id) })); };
  
  const handleSubmit = async (e) => {
    if (e) e.preventDefault(); setIsSaving(true);
    try {
      const { firestoreId, ...cleanData } = formData;
      if (editingOrder) await updateDoc(doc(db, 'artifacts', finalAppId, 'public', 'data', 'serviceOrders', editingOrder.firestoreId), cleanData);
      else await addDoc(collection(db, 'artifacts', finalAppId, 'public', 'data', 'serviceOrders'), cleanData);
  ...
  const handleSaveAndNewWithSameClient = async () => {
    ...
    try {
      ...
      setFormData({ ...formData, osNumber: generateNextOsNumber([...orders, formData]), item: '', manufacturer: '', model: '', serial: '', defect: '', solutionType: 'Manual', solution: '', solutionsList: [], notRepairableDetail: '', costThirdPartyName: '', costThirdPartyShipping: '', costClientShipping: '', costParts: '', chargedAmount: '', paymentCondition: 'À vista', installments: '', status: 'Recebido', trackingCode: '', sentToThirdParty: 'Não', thirdPartyInfo: '', thirdPartyTracking: '', thirdPartyDate: '' });
      setEditingOrder(null);
    } catch (err) { console.error(err); } finally { setIsSaving(false); }
  };

  const handleFilterByStatus = (status) => { setSelectedOrders(orders.filter(o => o.status === status).map(o => o.firestoreId)); };
  const handleFilterByBilling = (type) => { setSelectedOrders(orders.filter(o => o.billingType === type).map(o => o.firestoreId)); };
  ...
  const handlePrint = (printType) => {
    ...
    const content = `<html><head><title>${title}</title><style>@media print{@page{margin:1cm}}body{font-family:'Segoe UI',sans-serif;color:#333;line-height:1.4;padding:0;margin:0}.report-page{page-break-after:always;padding:20px;position:relative;min-height:27cm;border-bottom:1px solid #eee}.header{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #1a56db;padding-bottom:15px;margin-bottom:20px}.logo-area{color:#1a56db}.logo-text{font-size:28px;font-weight:900;margin:0}.logo-sub{font-size:10px;letter-spacing:2px;text-transform:uppercase;margin:0}.report-info{text-align:right}.report-title{font-size:16px;font-weight:900;color:#1a56db;text-transform:uppercase}.internal-badge{background:#b91c1c;color:white;padding:2px 6px;font-size:10px;border-radius:4px;font-weight:bold;margin-bottom:4px;display:inline-block}.section{margin-bottom:25px}.section-title{background:#f8fafc;padding:6px 12px;font-size:11px;font-weight:900;text-transform:uppercase;border-left:5px solid #1a56db;margin-bottom:12px;color:#1e40af}.client-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:12px;margin-bottom:20px}.items-table{width:100%;border-collapse:collapse;margin-top:10px}.items-table th{background:#f8fafc;text-align:left;padding:10px;font-size:10px;text-transform:uppercase;color:#64748b;border-bottom:2px solid #e2e8f0}.items-table td{padding:12px 10px;font-size:11px;border-bottom:1px solid #f1f5f9;vertical-align:top}.os-tag{font-weight:900;color:#1a56db;display:block;margin-bottom:4px}.cost-list-table{width:100%;margin-top:8px;border:1px solid #f1f5f9;font-size:10px;border-radius:4px}.cost-list-table th{background:#f8fafc;padding:4px 8px;font-size:9px}.cost-list-table td{padding:4px 8px;border-bottom:1px solid #f8fafc}.total-row{background:#f1f5f9;font-weight:bold}.internal-costs{margin-top:8px;padding:8px;background:#fff1f2;border:1px dashed #fda4af;border-radius:4px;font-size:10px;color:#9f1239}.signature-area{display:flex;justify-content:space-around;margin-top:80px}.signature-box{border-top:1px solid #333;width:250px;text-align:center;padding-top:8px;font-size:11px;font-weight:600}</style></head><body>${Object.values(groups).map(group=>`<div class="report-page"><div class="header"><div class="logo-area"><h1 class="logo-text">ALFA</h1><p class="logo-sub">TECNOLOGIA HOSPITALAR</p></div><div class="report-info">${printType==='internal'?'<div class="internal-badge">USO INTERNO - CONFIDENCIAL</div>':''}<div class="report-title">${printType==='internal'?'Relatório Gerencial':'Relatório de Atendimento'}</div><div style="font-size:10px;">Data: ${new Date().toLocaleDateString()}</div></div></div><div class="section"><div class="section-title">Dados do Cliente</div><div class="client-grid"><div><strong>Cliente:</strong> ${group.header.client}</div><div><strong>Atendimento:</strong> ${group.header.billingType} ${group.header.maintenanceVisit?'- '+group.header.maintenanceVisit:''}</div><div style="grid-column:span 2;"><strong>Endereço:</strong> ${group.header.address||'---'}</div></div></div><div class="section"><div class="section-title">Lista de Equipamentos</div><table class="items-table"><thead><tr><th width="15%">OS</th><th width="30%">Equipamento</th><th width="55%">Laudo Técnico</th></tr></thead><tbody>${group.items.map(item=>{const list=item.solutionsList||[];const total=list.reduce((acc,curr)=>acc+parseFloat(curr.cost.replace('.','').replace(',','.')||0),0);const internalBlock=printType==='internal'?`<div class="internal-costs"><strong>CONTROLE FINANCEIRO:</strong><br/>Cobrado: <b>R$ ${item.chargedAmount||'0,00'}</b> (${item.paymentCondition} ${item.installments?item.installments:''})<br/>Custos Operacionais: Frete Terceiro R$ ${item.costThirdPartyShipping||'0,00'} | Peças R$ ${item.costParts||'0,00'} | Frete Cliente R$ ${item.costClientShipping||'0,00'}</div>`:'';return`<tr><td><span class="os-tag">${item.osNumber}</span><small>${item.status}</small></td><td><strong>${item.item}</strong><br><div style="font-size:9px;color:#666;margin-bottom:2px;">${item.manufacturer||''} ${item.model||''}</div><small>NS: ${item.serial||'N/D'}</small></td><td><div><strong>Defeito:</strong> ${item.defect||'N/A'}</div><div><strong>Solução:</strong> ${item.solutionType==="Não passível de conserto"?`NÃO PASSÍVEL (${item.notRepairableDetail})`:(item.solutionType==="Preenchimento manual, com custo"?'Detalhamento abaixo:':(item.solution||'Em análise...'))}</div>${item.solutionType==="Preenchimento manual, com custo"&&list.length>0?`<table class="cost-list-table"><thead><tr><th style="text-align:left">Item / Serviço</th><th style="text-align:right">Valor</th></tr></thead><tbody>${list.map(s=>`<tr><td>${s.text}</td><td style="text-align:right">R$ ${s.cost}</td></tr>`).join('')}<tr class="total-row"><td>TOTAL</td><td style="text-align:right">R$ ${total.toLocaleString('pt-BR',{minimumFractionDigits:2})}</td></tr></tbody></table>`:''}${internalBlock}</td></tr>`}).join('')}</tbody></table></div><div class="signature-area"><div class="signature-box">Técnico Responsável</div><div class="signature-box">Cliente / Recebedor</div></div></div>`).join('')}<script>window.onload=function(){window.print();window.close()}</script></body></html>`;
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans overflow-hidden">
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-slate-900 text-white transition-all duration-300 flex flex-col fixed h-full z-30 shadow-2xl`}>
        ...
        <div className="flex flex-col border-t border-slate-800">
             {/* Atalho do olho na versão recolhida da sidebar */}
             {!isSidebarOpen && (
            ...
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-6 hover:bg-slate-800 flex justify-center">
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-64' : 'ml-20'} p-4 md:p-8 overflow-y-auto h-screen`}>
        ...
        {currentPage === 'inventory' && <div className="p-20 text-center"><Boxes size={80} className="mx-auto text-slate-200 mb-6"/><h3 className="text-2xl font-black">Inventário em breve</h3></div>}
        {currentPage === 'clients' && <div className="p-20 text-center"><Users size={80} className="mx-auto text-slate-200 mb-6"/><h3 className="text-2xl font-black">Módulo de Clientes em breve</h3></div>}
      </main>
      ...
      {isModalOpen && (
         ...
         <div className="bg-white rounded-[2.5rem] w-full max-w-5xl my-auto shadow-2xl border border-slate-200 animate-in zoom-in-95 duration-200">
            <form onSubmit={handleSubmit} className="p-6 md:p-10 space-y-10">
               {/* ... (CONTEÚDO DO FORMULÁRIO OS - MANTIDO) ... */}
               ...
               <div className="space-y-10">
                  <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 space-y-8">
                     <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input placeholder="Nome da Empresa" className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-bold" value={formData.client} onChange={e => setFormData({...formData, client: e.target.value})} />
                            <input placeholder="CNPJ" className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-bold" value={formData.cnpj} onChange={e => setFormData({...formData, cnpj: e.target.value})} />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input placeholder="Responsável" className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-bold" value={formData.contactPerson} onChange={e => setFormData({...formData, contactPerson: e.target.value})} />
                            <input placeholder="E-mail" className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-bold" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                        </div>
                        <div className="space-y-1">
                           ...
                           <input placeholder="Endereço Completo" className="w-full p-4 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 transition-all font-bold" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                        </div>
                     </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <input placeholder="Item / Nome" className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold" value={formData.item} onChange={e => setFormData({...formData, item: e.target.value})} />
                        <input placeholder="Marca" className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold" value={formData.manufacturer} onChange={e => setFormData({...formData, manufacturer: e.target.value})} />
                        <input placeholder="Modelo" className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold" value={formData.model} onChange={e => setFormData({...formData, model: e.target.value})} />
                        <input placeholder="Número de Série" className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold font-mono" value={formData.serial} onChange={e => setFormData({...formData, serial: e.target.value})} />
                     </div>
                  </div>
                  <div className="space-y-6">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <textarea rows="8" placeholder="Descrição detalhada do defeito..." className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-medium" value={formData.defect} onChange={e => setFormData({...formData, defect: e.target.value})} />
                        <div className="space-y-4">
                           ...
                           {formData.solutionType === "Manual" && <textarea rows="4" placeholder="Descrição da solução aplicada..." className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-medium" value={formData.solution} onChange={e => setFormData({...formData, solution: e.target.value})} />}
                           ...
                           {formData.solutionType === "Não passível de conserto" && (
                              <select className="w-full p-4 bg-red-50 text-red-900 border border-red-100 rounded-2xl outline-none font-bold" value={formData.notRepairableDetail} onChange={e => setFormData({...formData, notRepairableDetail: e.target.value})}>
